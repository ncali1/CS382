...
int contains(struct int_array* arr, int target) {
	...
	for (i = 0; i < arr->count; ++i) // remove semi colon
	{
...
void add(struct int_array* arr,  int payload) {
	...
	//add - 1 to fix leading column of 0's
	arr->data[(++ arr->count )- 1] = payload;
}
int remove_elem(struct int_array* arr,  int target) {

	unsigned int i = 0;

	if ((arr->count == 0))// make it ==
	...
	if (i == arr->count) //remove semi colon
